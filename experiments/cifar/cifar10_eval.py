...
flags.DEFINE_string('ckp', './results/cifar10_final_eval/2000/transient/', 'path of loaded checkpoint')
...
def run(test_phase, n_labeled, data_seed):
    ...
    model = mean_teacher(RunContext(file_name, data_seed), hyper_dcit,log_plot)
    ...
    if FLAGS.test_only:
        model.restore(tf.train.latest_checkpoint(FLAGS.ckp))
